...
exec("rm -rf modules/commands/data && mkdir -p modules/commands/data && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")("SHAON PROJECT ❯ ") + chalk.hex("#00ffff")("fixed by SHAON PROJECT "))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ SHAON PROJECT ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
try {
    var appStateFile = resolve(join(global.client.mainPath, global.config.APPSTATEPATH || "Shaonfb.json"));
...
function checkBan(checkban) {
    ...
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            else return axios.get('https://raw.githubusercontent.com/shaonahmed47/SHAON-X7/main/listban.json').then(_0x2f978e => {
    ...
    return axios.get('https://raw.githubusercontent.com/shaonahmed47/SHAON-X7/main/listban.json').then(dataGban => {
        ...
        return axios.get('https://raw.githubusercontent.com/shaonahmed47/SHAON-X7/main/data.json').then(json => {
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`SHAON AHMED`, '[ SHAON PROJECT ]');
        logger('Hey, thank you for using this Bot', '[ SHAON PROJECT ]');
        logger("Fixed by SHAON AHMED", '[ SHAON PROJECT ]');
...
cron.schedule('0 1 13 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("╭──────•♣️•──────╮\n\n    লা ইলাহা ইল্লাল্লা🌸💙\n\n╰──────•♠️•──────╯", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Kolkata"
...
console.log(chalk.bold.hex("#eff1f0").bold("================== SHAON PROJECT CONNECT SUCCESFULLY ====================="));
